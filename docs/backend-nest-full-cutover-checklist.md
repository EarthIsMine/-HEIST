...
- [ ] 전환 전용 브랜치 생성 (`feat/nest-full-cutover`)
- [ ] 현재 동작 상태 태그 또는 커밋 SHA 기록
- [ ] `.env`/배포 변수 스냅샷 저장
...
- [ ] Nginx/Ingress 설정에서 legacy 업스트림 의존 제거
- [ ] WebSocket 업그레이드 경로가 Nest 포트로 향하는지 검증
- [ ] 로컬/스테이징 포트 충돌 여부 확인

완료 기준:
- [ ] HTTP + WS 모두 Nest만 타고 동작한다.

## 4) 기능 동등성 스모크 테스트

- [ ] 로비 진입/방 생성/방 참가
- [ ] 게임 시작/틱 진행/결과 처리
- [ ] 재연결(소켓 끊김 후 재입장) 시나리오
- [ ] 운영 엔드포인트 (`/health`, `/_metrics`, `/_state/*`) 응답 검증
- [x] `scripts/canary-smoke.sh` 재사용 가능 시 Nest 대상 실행

완료 기준:
- [ ] 사용자 핵심 플로우에서 blocker 이슈가 없다.
...
완료 기준:
- [ ] 다음 개발 사이클에서 legacy 없이 기능 개발 가능하다.

## 권장 커밋 분할

- [ ] Commit A: `feat: 실행 엔트리/문서 전환`
- [ ] Commit B: `refactor: 라우팅 및 카나리 분기 단순화`
- [ ] Commit C: `fix: 레거시 backend 제거 후 CI/배포 경로 정리`
