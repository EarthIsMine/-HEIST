export const MAP_WIDTH = 2400;
export const MAP_HEIGHT = 2400;
export const PLAYER_RADIUS = 28;

export const TICK_MS = 50;
export const MATCH_DURATION_MS = 5 * 60 * 1000;
export const HEAD_START_MS = 5 * 1000;

export const PLAYER_SPEED = 260;

export const STEAL_RANGE = 90;
export const STEAL_RATE = 5;
export const BREAK_JAIL_RANGE = 110;
export const BREAK_JAIL_CHANNEL_MS = 5_000;
export const ARREST_RANGE = 75;
export const ARREST_COP_COUNT = 1;
export const ARREST_STUN_MS = 2_000;

export const TOTAL_COINS = 300;
export const STORAGES_COUNT = 6;
export const COINS_PER_STORAGE = 50;
export const ENTRY_FEE_LAMPORTS = 0;
export const MAX_PLAYERS = 6;
export const COP_COUNT = 2;
export const THIEF_COUNT = 4;
export const STEAL_MOVE_SPEED_MULTIPLIER = 0.4;
export const COP_VISION_RADIUS = 280;
export const THIEF_VISION_RADIUS = 220;

// Disguise skill
export const DISGUISE_DURATION_MS = 10_000;
export const DISGUISE_COOLDOWN_MS = 30_000;

// Wall building skill
export const WALL_COST_COINS = 10;
export const WALL_DURATION_MS = 15_000;
export const WALL_COOLDOWN_MS = 10_000;
export const WALL_WIDTH = 120;
export const WALL_HEIGHT = 20;
export const WALL_PLACE_DISTANCE = 60;
